...
Creates and runs the container. The option `--rm` makes it so that the container is removed after it exits. After ` podman run` executes the command ` podman ps -a` lists the available containers. Notice that the nodejs container is not listed. This is because the `-rm` option was used. The `nodejs` container spun up and then because there was no activities to it to run, it exited. Once the container exited, it was removed from the local machine.
