...
After the container is created, the command `podman ps -a` is executed to list the containers running on the local machine. Notic the the `nginx` container has the name `mywebserver`.
...
Creates and runs the container. The option `--rm` makes it so that the container is removed after it exits. After ` podman run` executes the command ` podman ps -a` lists the available containers. Notice that the nodejs container is not listed. This is because the `-rm` option was used. The `nodej`s` container spun up and then because there was no activities to it to run, it exited. Once the container exited, it was removed from the local machine.
...
The following example lists all containers on the local machine using `podman ps -a`. Notice that the two container listed have a status of `Up ## minutes ago`. The command `podman stop mywebserver` is then executed against the container with the name `podman stop mywebserver`.
...
The following examples uses `podman run` to run the container image `docker.io/library/nginx`. The `-d` option is used to run the container in the background. The `--name` is used to give the container the name `mywebserver`. The `-p` option is used to assign port number `8181` on localhost to the port number `80` which is the port number on which the nginx web server within the container is listening for income requests.
...
----
podman create </repo/image_name:tag>
----
