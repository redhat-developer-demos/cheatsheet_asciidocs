...
`sed [options] <manipulation instructions> <path/to/filename or stduout>`
...
The program `traceroute` is not part of Red Hat Linux by default. It must be installed using `sudo yum install traceroute`.
