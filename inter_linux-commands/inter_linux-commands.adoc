...
The following example shows the command `du` to report the amount of disk space used by the files in the directory `/etc/bin`. (`$` is the command-line prompt symbol.)
