...
The following uses `dnf module reset` along with `dnf module install` to install the module `nodjs:16` over an existing, older version of Node.js. The results are shown in abbreviated portions in order to economize on displaying screen output.
