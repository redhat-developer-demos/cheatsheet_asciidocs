...
* log in to a container image registry
...
The following example creates a new tag, `verylatest` and applies it to the existing container image `docker.io/library/nginx` that as the tag `latest`. Notice that the values of the `IMAGE ID`` are identical.
...
The following example creates 3 working containers using the `buildah from` command. Then, the working directories are listed using the `buildah containers` command.
...
=== Delete all containers on a machine 1
...
=== Delete all containers on a machine 2
