...
The following example uses the `systemctl status` to report status information about the `sshd` service. `systemctl status` also displays information about the service's activities via log entries that follow the status information.
...
The following example uses the `journalctl` command to display log information stored under systemd in binary format. The `--follow` option is used to only the most recent journal entries.
