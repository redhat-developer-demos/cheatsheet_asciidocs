...
The following adds a user with the login name `cooluser`. The `HOME` directory `home/cooluser` is created by default. Then the example users the command `passwd` to set a password for the new user.
