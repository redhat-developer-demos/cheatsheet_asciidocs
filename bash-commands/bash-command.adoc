...
A Bash script is a text file that contains programming statements that execute tasks operating system commands. Typically system administrators and programmers use Bash scripts to avoid having to repetitively execute tasks manually in a terminal.

Bash scripts are typically used for computer set up activities. Thus, in addition to be ing a tool for system administrators and programmers, a Bash script can also be used by system provisioning software.
...
Unlike a binary executable file which knows how to interact with the computer's operating system directly, a text-based bash script requires another program to run its commands. This other program in called an interpreter. The interpreter that runs a Bash script is, as the name implies, `bash`. However, in other cases, a bash script can be run by another interpreter that's installed on the host computer. An example of another interpreter is `sh`.
...
The shebang for declaring a Perl script is

```
#!/usr/env perl
```

Be advised that it is entirely possible to run a script written in another language as long as long as the operating system knows which language interpreter to use. For example, you can write a program in the Perl programming language and use the associated interpreter.
