...
There are two ways to run a bash script. The first way is to execute it as a parameter of a direct call to the the binary file `bash`, like so:
...
The second way to use the `chmod` command to change the permissions of the bash script to include running it as an executable, like so:
...
Unlike a standalone binary executable which knows how to interact with the computer's operating system, as shown above, a bash script requires another program to run it. This other program in called an interpreter. The interpreter that runs a bash script is, as the name implies, `bash`. However, in many cases, a bash script can be run by another interpreter that's installed on the host computer. An example of another interpreter is `sh`.

Also, it is entirely possible to run a script that uses an interpreter such as Perl. (Of course, the script needs to written in the Perl programming language.)

The way a bash script lets the operating system know which interpreter to use is according to a declaration made at the first line of the script. This first line declaration is called a script header. It is also called a https://en.wikipedia.org/wiki/Shebang_(Unix)[shebang]. A shebang starts with the character `#!`
...
The shebang shown above tells the operating system to use the interpreter located at the filepath `/usr/bin/bash` to execute the lines of code that will follow in the script. In this case the interpreter is `bash`.
...
Unlike the shebang in the first example that explicitly tells the operating system where to locate the interpreter in file system, the shebang `#!/usr/env bash` tells the operating system to search the computer's `$PATH` to find the `bash` interpreter. Thus, any intance of `bash` can be used as long as the it's in a location defined by the `$PATH` environment variable.

The example below illustrates using a script header that tells the operating system to execute the file as a Perl script:
...
Being able to use variables is a critical need for programming Bash scripts. The following sections describe various aspect of working with variables under Bash.
